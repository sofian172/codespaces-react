...
function App() {
  ...
  async function unlock(e) {
    ...
    if (value === '9c9a1d72b15c7c54d3b7f1a4e6c4f6e0a1e5b8b0f8d9d6a4d1f0d3d5e5f5e3e2') {
  ...
  async function sendMessage(text = message) {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: next, files }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'تعذر الاتصال بالخادم');
      setMessages((m) => [...m, { role: 'assistant', content: data.text }]);
      if (messages.length === 0) {
        setChats((items) => [{ id: Date.now(), title: clean.slice(0, 34), preview: clean }, ...items].slice(0, 20));
      }
    } catch (err) {
      setMessages((m) => [...m, { role: 'assistant', content: `⚠️ ${err.message}\n\nإذا لم تضف AI_GATEWAY_API_KEY في Vercel، أضفها من Environment Variables.` }]);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function runTerminal() {
    ...
    const data = await res.json();
    setTerminal((t) => [...t, `$ ${input}`, data.output || data.error]);
    setTab('terminal');
  }
  ...
  return (
    <div className="app-shell" dir="rtl">
      ...
      <section className="workspace">
        <header className="topbar"><div><span className="status-dot"/> Online <span className="divider">·</span> GPT-5.2 Fast</div><button onClick={() => alert('Connections: GitHub و Vercel — أضف التوكنات من إعدادات Vercel/البيئة عند تشغيل التكاملات.')}>⚙️</button></header>
